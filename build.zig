//! Improvement build.zig written by kprotty
//! https://github.com/kprotty/duckdb/blob/zig-build/build.zig - origin
...
pub fn build(b: *std.Build) void {
    ...
    // TODO: Git parsing
    const git = b.findProgram(&.{"git"}, &.{}) catch @panic("Not found git executable.");
    const git_commit_hash = std.process.Child.exec(
        .{
            .allocator = b.allocator,
            .argv = &.{ git, "rev-parse", "HEAD" },
        },
    ) catch @panic("Cannot run git command.");
    ...
    cxx_flags.appendSlice(&[_][]const u8{
        ...
        "-ffunction-sections",
        "-fdata-sections",
        b.fmt("-DDUCKDB_VERSION=\"v{}.{}.{}-dev{}\"", .{
            version.major, version.minor, version.patch, 0,
        }),
        b.fmt("-DDUCKDB_SOURCE_ID=\"{s}\"", .{
            git_commit_hash.stdout[0 .. git_commit_hash.stdout.len - 1],
        }),
    }) catch unreachable;
    ...
    for (find_cmake_files(b, "third_party")) |path| {
        ...
        if (std.mem.endsWith(u8, dirname, "icu")) continue;
...
fn find_cmake_files(b: *std.Build, folder: []const u8) []const []const u8 {
...
fn parse_cmake_call(b: *std.Build, data: []u8, prefix: []const u8) ?[][]const u8 {
